...
tasks are easily tackled by using a real scripting laguage (like :s). 
...
#+END_SRC
 ...
 - Permissions: to run a script file in Bash, you need to give excecution permissions, we can add
...
session is too short for covering ~emacs~, but we can easily talk about ~vim~, a poweful text editor
that can replace any IDE feature, and that will help you to write better code. 

Let's open Vim by running ~vi~ or ~vim~ in your terminal. Once you open it, try to exit it ... :astonished: 
