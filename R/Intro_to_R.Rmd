---
title: "EPIC Orientation 2019: Introduction to R"
...
* #### vector
    ...
    + elements of a vector must be of the same type
      + `numeric_vector <- c(0.1, 2, 4.678)`
      + `character <- c("EPIC", "is", "epic!")`
      + `logical <- c(T, F, T, T)`
      + factor: categorical variable with predefined set of possible values ("levels") and, optionally, a rank order. Looks like character strings, but internally, the levels are mapped to integers
* #### matrix
    + 2-D vector
    + all columns in a matrix must be of the same type (numeric, character, etc.) and the same length
* #### dataframe
    + more general than a matrix, different columns can have different types (numeric, character, factor, etc.)
* #### list
...
`x %>% some_fn(...)` is equivalent to `some_fn(x,...)`
